...
function buildRawIntent(companyName: string | null, state: WizardState) {
  ...
  lines.push(
    `- Kanalen: ${Object.entries(state.channels)
      .filter(([, on]) => on)
      .map(([k]) => k)
      .join(", ")}`
  );
...
export default function CreateClient() {
  ...
  const companyName =
    mode === "live" ? liveCompany?.name ?? null : demoCompany?.name ?? null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const inputObj =
          input && typeof input === "object" ? (input as Partial<WizardState>) : null;
        setState((prev) => ({
          ...prev,
          ...basePatch,
          ...(inputObj ?? {}),
  ...
  const brandOk =
    mode === "demo" ? Boolean(demoCompany?.brandTone) : isBrandComplete(liveCompany);
      ...
      const res = await fetch(
        ...
        await withAuth({
          ...
          body: JSON.stringify({
            text,
            tone:
              (mode === "live" ? liveCompany?.brand_tone : demoCompany?.brandTone) ??
              undefined,
          }),
        })
      );
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setState((p) => ({ ...p, location: e.target.value }))
                          }
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setState((p) => ({ ...p, seniority: e.target.value }))
                          }
                          className="mt-2 w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-zinc-400 dark:border-zinc-800 dark:bg-black dark:focus:border-zinc-600"
                        >
                      ...
                      <div className="mt-2 grid gap-3 md:grid-cols-2">
                        <textarea
                          ...
                          onChange={(e) =>
                            setState((p) => ({
                              ...p,
                              responsibilitiesFreeText: e.target.value,
                            }))
                          }
                          rows={6}
                  ...
                  <>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        />
                        <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                          Default gebruikt Joppa jullie bedrijfsstijl. Hier kun je afwijken per
                          vacature.
                        </p>
                      </div>
                      <div>
                    ...
                    <div>
                        ...
                        {(
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              disabled={disabled}
                              onClick={() => setState((p) => ({ ...p, visualTemplate: t.id }))}
                      ...
                      <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                        In v1 gebruiken we templates (super goedkoop en consistent). AI-beelden voegen
                        we later toe met “regenereer” knop.
                      </p>
                    </div>
                  </>
                ) : null}
                ...
                {stepIdx === 5 ? (
                  <>
                    <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-black dark:text-zinc-200">
                      <div className="font-semibold text-zinc-900 dark:text-zinc-100">Check</div>
                      <ul className="mt-2 space-y-1 text-sm">
                        ...
                        <li>
                          <b>Werk</b>:{" "}
                          {state.responsibilities.length >= 3 ? "ok" : "min. 3 bullets"}
                        </li>
                        <li>
                          <b>Eisen</b>:{" "}
                          {state.mustHaves.length >= 3 ? "ok" : "min. 3 must-haves"}
                        </li>
                        <li>
                          <b>Kanalen</b>:{" "}
                          {state.channels.website || state.channels.indeed
                            ? "ok"
                            : "zet website of indeed aan"}
                        </li>
                      </ul>
                    </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const generated = mockGenerateCampaign({
                            rawIntent,
